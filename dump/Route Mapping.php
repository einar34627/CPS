...
<body>
    <div class="page">
        ...
        <div class="stats-grid">
            ...
            <button class="btn btn-outline" id="toggle-traffic">
                <i class="bx bx-traffic-cone"></i> Traffic
            </button>
        <div class="overlay">
            <div class="panel">
...
/* =====================================================
   EXACT COMMONWEALTH QC BOUNDARY (IRREGULAR GEOJSON)
===================================================== */
const commonwealthGeoJSON = {
  "type": "Feature",
  "properties": { "name": "Commonwealth QC" },
  "geometry": {
    "type": "Polygon",
    "coordinates": [[
      [121.0789,14.7056],
...
L.polyline([
  ...
  weight: 7,
  opacity: 0.9
}).bindPopup('Commonwealth Avenue').addTo(map);
...
/* =====================================================
   BUSY ZONE (CLIPPED SA BOUNDARY)
===================================================== */
L.geoJSON(commonwealthGeoJSON, {
    style: {
        fillColor: '#f97316',
        fillOpacity: 0.20,
        color: '#f97316'
    }
}).bindPopup('High Traffic / High Incident Zone')
  .addTo(map);
...
const incidentHeat = L.heatLayer([
    [14.7018,121.0862,0.9],
    [14.6996,121.0850,0.7],
    [14.7028,121.0885,0.8],
    [14.6989,121.0836,0.6],
    [14.7039,121.0890,0.75]
], {
    radius: 30,
    blur: 20,
    maxZoom: 18
...
[
  [14.7011,121.0860,'CCTV-01'],
  [14.6999,121.0842,'CCTV-02'],
  [14.7027,121.0884,'CCTV-03']
].forEach(c => {
    L.marker([c[0],c[1]],{icon:cctvIcon})
        .bindPopup(`<b>${c[2]}</b><br>Status: Active`)
        .addTo(map);
});

/* =====================================================
   PATROL ROUTE DRAWING (INSIDE ONLY)
===================================================== */
let routePoints = [];
...
map.on('click', e => {
    if (!commonwealthZone.getBounds().contains(e.latlng)) return;
...
/* =====================================================
   REAL-TIME PATROL GPS (SIMULATED)
===================================================== */
...
const patrolMarker = L.marker([14.6964,121.0854], {
    icon: patrolIcon
}).addTo(map).bindPopup('Patrol Unit Alpha');
...
let i = 0;
setInterval(() => {
    patrolMarker.setLatLng(patrolPath[i]);
    i = (i + 1) % patrolPath.length;
}, 3000);

/* =====================================================
   AI PATROL PRIORITY SCORING
===================================================== */
const hour = new Date().getHours();
const priorityScore =
    (incidentHeat._latlngs.length * 0.6) +
    (hour >= 7 && hour <= 10 ? 2 : 1) +
    (hour >= 17 && hour <= 20 ? 2 : 0);

L.popup({ closeButton:false })
.setLatLng([14.7016,121.0866])
.setContent(`
<b>AI Patrol Priority</b><br>
Score: <b>${priorityScore.toFixed(1)}</b><br>
Action: ${priorityScore > 4 ? 'Immediate Patrol Required' : 'Routine Monitoring'}
`)
.openOn(map);
...
map.on('locationfound', function(e){
    L.circleMarker(e.latlng,{
      ...
      .bindPopup('Patrol Unit Location');
});
...
legend.onAdd = function(){
    ...
    div.innerHTML = `
        ...
        <span style="color:#fb923c;">■■■</span> Busy Area<br>
        <span style="color:#16a34a;">━━━</span> Patrol Route
    `;
    return div;
};
legend.addTo(map);
